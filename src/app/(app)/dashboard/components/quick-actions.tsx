...
function QuickActionCard({ action }: QuickActionCardProps) {
  ...
  return (
    <Link
      href={action.href}
      className="group flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] hover:border-white/10 transition-all"
    >
      {/* Icon */}
      <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", action.color)}>
...
export function QuickActions({ className }: QuickActionsProps) {
  return (
    <div className={cn("space-y-3", className)}>
      {/* Header */}
      <h3 className="text-sm font-medium text-white px-1">Ações Rápidas</h3>
