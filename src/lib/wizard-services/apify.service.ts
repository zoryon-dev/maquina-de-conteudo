 ...
 *   // → Returns structured string with all video metadata + transcript
 * }
 * ```
 */
export function formatYouTubeForPrompt(
  ...
  parts.push("═══════════════════════════════════════════════════════════════");
  parts.push("VÍDEO DO YOUTUBE ANALISADO");
  parts.push("═══════════════════════════════════════════════════════════════");
  parts.push("");

  // Basic video info
  if (transcription.metadata?.title) {
    parts.push(`📺 TÍTULO: ${transcription.metadata.title}`);
  }
  if (transcription.metadata?.channelName) {
    parts.push(`👤 CANAL: ${transcription.metadata.channelName}`);
  }
  ...
  if (transcription.metadata?.commentCount) {
    metrics.push(`${formatNumber(transcription.metadata.commentCount)} comentários`);
  }
  if (metrics.length > 0) {
    parts.push(`📊 ENGAJAMENTO: ${metrics.join(" • ")}`);
  }

  // Channel authority
  if (transcription.metadata?.subscriberCount) {
    parts.push(`👥 INSCRITOS: ${formatNumber(transcription.metadata.subscriberCount)}`);
  }
  ...
  // URL
  parts.push(`🔗 LINK: ${transcription.sourceUrl}`);
  parts.push("");
